...
from local_eval import evaluate_functional_correctness
from humaneval import ensure_humaneval_file
from mbpp import ensure_mbpp_problem_file
import tempfile
...
def _math_verify_equal(gold_text, pred_text, timeout_s=5):
    """Robust math equivalence using math_verify if available.

    Tries to use `math_verify.parsed` which can extract final answers from free-form
    model generations. Falls back to `math_verify.parse` if needed.

    Returns False on any failure, so callers should provide fallbacks.
    """
    try:
        try:
            from math_verify import parsed as mv_parse, verify  # type: ignore
        except Exception:
            from math_verify import parse as mv_parse, verify  # type: ignore

        gold_obj = mv_parse(str(gold_text))
        pred_obj = mv_parse(str(pred_text))
...
def _numeric_equal(a, b, rel_tol=1e-6, abs_tol=1e-8):
    """Numeric comparison with tolerance; safe-casts to float if possible."""
    try:
        fa = float(a)
        fb = float(b)
        return abs(fa - fb) <= max(abs_tol, rel_tol * max(abs(fa), abs(fb), 1.0))
    except Exception:
        return False
...
def parse_gsm_answers(json_path=None, json_data=None):
    ...
    for item in data.get("generations", []):
        ...
        # First, try robust check directly on full generation text
        is_correct = _math_verify_equal(ground_truth, raw_generation)
        if not is_correct and parsed_answer is not None:
            # Fallback: verify on extracted numeric/boxed answer, else tolerant numeric compare
            is_correct = _math_verify_equal(ground_truth, parsed_answer) or _numeric_equal(
...
def parse_math_answers(json_path=None, json_data=None):
    ...
    for item in data.get("generations", []):
        ...
        is_correct = _math_verify_equal(ground_truth, raw_generation)
        if not is_correct and parsed_answer is not None:
            # Fallback: check parsed extraction then helper equivalence
            is_correct = _math_verify_equal(ground_truth, parsed_answer) or is_equiv(
                parsed_answer, ground_truth
            )
...
def parse_humaneval_answers(json_path=None, json_data=None):
    ...
    # Prepare temporary samples file for execution-based checking
    ...
    try:
        ...
        # Run local evaluator; produces .scored.jsonl
        ...
        # Build a map from task_id to passed
        passed_map = {}
        if os.path.exists(scored_path):
            with open(scored_path, "r", encoding="utf-8") as sf:
                for line in sf:
                    rec = json.loads(line)
                    passed_map[rec.get("task_id")] = bool(rec.get("passed", False))
        ...
        for item in data.get("generations", []):
            ...
            total_effective_tokens += effective_tokens
            task_id = item.get("task_id")
            is_correct = passed_map.get(task_id, False)
            if is_correct:
                total_correct += 1
...
def parse_mbpp_answers(json_path=None, json_data=None):
    ...
    try:
        ...
        evaluate_functional_correctness(
            input_file=tmp_path,
            n_workers=8,
            problem_file=ensure_mbpp_problem_file(),
            is_mbpp=True,
            language="python",
            out_dir=os.path.dirname(json_path) if json_path else ".",
        )
    ...
    for json_file in json_files:
        ...
        if setup_name:
            ...
            if "gsm" in setup_name:
                ...
                # Unsupported dataset (e.g., humaneval, mbpp) – skip to avoid unbound variables
                continue
...
if __name__ == "__main__":
    ...
    if __name__ == "__main__":
        parser = argparse.ArgumentParser(description="Aggregate results from a directory of eval outputs.")
        parser.add_argument(
            "--dir",
            type=str,
            default="/home/ywxzml3j/ywxzml3juser46/diffusion_LM/d1/eval/eval_results_p2_pa_code",
