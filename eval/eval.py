...
if __name__ == "__main__":
    ...
    parser.add_argument("--csv_path", type=str, default="eval_results/summary.csv")
    ...
    if args.checkpoint_path:
        ...
        if args.merge_lora:
            # Merge LoRA weights into the base model for evaluation
    ...
    # Ensure all ranks wrote their outputs
    if dist.get_world_size() > 1:
        dist.barrier()

    # Rank 0: aggregate current run and append to CSV
    if dist.get_rank() == 0 and not args.dont_save:
        os.makedirs(os.path.dirname(args.csv_path), exist_ok=True)
        pattern = f"{args.output_dir}/{args.dataset}_{model_name}_{args.gen_length}_{args.diffusion_steps}_*_generations.json"
